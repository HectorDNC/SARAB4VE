...
export default function OrganizationDetailPage() {
    ...
    const [organizacion, setOrganization] = useState<ApiUser | null>(null);
    ...
    useEffect(() => {
        async function loadOrganizacion() {
            ...
            if (USE_MOCK) {
                const found = MOCK_USERS.find((v) => v.id === id && v.role == 'organization');
            ...
            try {
                ...
                setOrganization(user as ApiUser)
            } catch (error) {
    ...
    const handleDecision = async (status: "approved" | "rejected") => {
        ...
        if (status === "approved") {
            try {
                ...
                alertService.error(`Error al registrar aprovación de la organización: ${error}`);
                return;
            }
        } else {
            try {
                ...
                alertService.error(`Error al registrar rechazo  de la organización: ${error}`);
                return;
            }
        }
        ...
        return (
            <div className="mx-auto mt-6">
                <div className="rounded-2xl border border-outline-variant bg-surface-container-low p-6">
                    ...
                    <Link href="/dashboard/organizaciones" className="text-primary flex justify-center font-semibold text-sm mt-4">
                        Volver al listado
                    </Link>
                </div>
            </div>
        ...
        <div className="mx-auto mt-6">
            ...
            <div className="rounded-2xl border border-outline-variant bg-surface-container-low p-6">
                <div className="flex items-center gap-3 flex-wrap">
                    <h1 className="text-xl sm:text-2xl font-bold text-on-surface">{organizacion.fullName}</h1>
                    <StatusBadge status={organizacion.status} />
                </div>
                ...
                <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl border border-outline-variant bg-background p-4">
                        ...
                        <div className="mt-1.5 flex items-center gap-1.5">
                            <p className="text-sm font-medium text-on-surface truncate">{organizacion.email}</p>
                            {organizacion.emailVerified ? (
                    ...
                    <div className="rounded-xl border border-outline-variant bg-background p-4">
                        ...
                        <div className="mt-1.5 flex items-center gap-1.5">
                            <p className="text-sm font-medium text-on-surface">{organizacion.phone}</p>
                            {organizacion.phoneVerified ? (
                                <span className="material-symbols-rounded text-base text-[color:var(--color-success)] shrink-0" title="Verificado" aria-hidden="true">
                                    verified
                                </span>
                            ) : (
                    ...
                    {organizacion.location && (
                        <div className="rounded-xl border border-outline-variant bg-background p-4 sm:col-span-2">
                            ...
                            <Location value={organizacion.location} readOnly />
                            <p className="text-xs text-on-surface-variant mt-2">
                                {organizacion.location.lat.toFixed(5)}, {organizacion.location.lng.toFixed(5)}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="rounded-xl border border-outline-variant bg-background p-4 sm:col-span-2">
                        ...
                        <p className="mt-1.5 text-sm font-medium text-on-surface">{organizacion.zone}</p>
                    </div>
                    ...
                    <div className="rounded-xl border border-outline-variant bg-background p-4 sm:col-span-2">
                        ...
                        <p className="mt-1.5 text-sm font-medium text-on-surface">
                            {new Date(organizacion.createdAt).toLocaleDateString("es", {
                                day: "numeric",
                                month: "long",
                                year: "numeric",
                            })}
